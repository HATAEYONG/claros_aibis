...
export const ERPSourceManagement: React.FC = () => {
  ...
  const fetchSources = async () => {
    ...
    try {
      ...
      console.warn('ERP sources API 연동 실패, 샘플 데이터 사용:', error);
      // 시연용 샘플 데이터
      setSources([
        {
          erp_source_id: 1,
          source_code: 'YH',
          source_name: '유한양행 ERP',
          source_type: 'postgresql',
          description: '주 생산 시스템 - SAP 기반',
          host: 'erp.yuhan.co.kr',
          port: 5432,
          database_name: 'yuhan_prod',
          is_default: true,
          is_active: true,
          table_count: 125,
        ...
        },
        {
          ...
          source_name: 'FOM 시스템',
          source_type: 'mssql',
          description: '원자재 관리 시스템',
          host: 'fom.internal',
          port: 1433,
          database_name: 'fom_db',
          is_default: false,
          is_active: true,
          table_count: 87,
        ...
        },
        {
          ...
          host: 'mes.prod.yuhan.co.kr',
          port: 1521,
          database_name: 'mes_prod',
          is_default: false,
          is_active: false,
          table_count: 203,
        ...
        },
        {
          erp_source_id: 4,
          source_code: 'WMS',
          source_name: '창고 관리 시스템',
          source_type: 'mysql',
          description: '물류 및 창고 관리',
          host: 'wms.internal',
          port: 3306,
  ...
  const handleTestConnection = async (sourceId: number) => {
    ...
    try {
      ...
      // 시연용 샘플 응답
      const source = sources.find(s => s.erp_source_id === sourceId);
      const isSuccess = source?.is_active !== false;

      setTestResult({
        status: isSuccess ? 'success' : 'error',
        message: isSuccess
          ? `${source?.source_name} 연결 성공 (응답시간: ${Math.floor(Math.random() * 100 + 50)}ms)`
          : `${source?.source_name} 연결 실패 - 호스트를 확인해주세요`,
        connection_time: Math.floor(Math.random() * 100 + 50),
        database_accessible: isSuccess,
        tables_count: source?.table_count || 0,
