/**
 * ERP 동기화 서비스 설정 컴포넌트
 * SAP/FOM ERP 동기화 서비스의 On/Off 설정을 관리하는 UI
 */
...
export const ERPSyncConfiguration: React.FC = () => {
  ...
  const [allServicesDisabled, setAllServicesDisabled] = useState(false);
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      // Check if all ERP services (sap, fom) are disabled
      const erpServices = data.services.filter((s: ERPSyncServiceConfig) =>
        s.service_type === 'sap' || s.service_type === 'fom'
      );
      const allDisabled = erpServices.every((s: ERPSyncServiceConfig) => !s.is_enabled);
      setAllServicesDisabled(allDisabled);
    } catch (error) {
  ...
  const handleGenerateSampleData = async () => {
    if (!confirm('샘플 데이터를 생성하시겠습니까? 이 작업은 모든 ERP 서비스가 비활성화된 경우에만 가능합니다.')) return;

    setGeneratingSampleData(true);
    try {
      const response = await fetch('/api/erp-sync/service-config/sample/generate/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleActivateSampleService = async () => {
    setTogglingService('sample');
    try {
      // toggle 엔드포인트 사용 (이미 sample 서비스를 처리하도록 수정됨)
      const response = await fetch(`/api/erp-sync/service-config/toggle/sample/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to activate sample service');
      }

      const result = await response.json();
      alert(result.message || '샘플 데이터 서비스가 활성화되었습니다.');
      fetchServices();
    } catch (error: any) {
      console.error('Error activating sample service:', error);
      alert(error.message || '샘플 데이터 서비스 활성화에 실패했습니다.');
    } finally {
      setTogglingService(null);
    }
  };
    ...
    <div className="p-6 bg-white rounded-lg shadow">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-amber-800 mb-1">
                모든 ERP 동기화 서비스가 비활성화되어 있습니다
              </h3>
              <p className="text-xs text-amber-700 mb-3">
                샘플 데이터 서비스를 활성화하여 시스템을 테스트할 수 있습니다.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={handleGenerateSampleData}
                  disabled={generatingSampleData}
                  className="flex items-center gap-2 px-3 py-1.5 bg-amber-600 text-white text-sm rounded-lg hover:bg-amber-700 disabled:bg-gray-400 transition-colors"
                >
                  <Play className="w-4 h-4" />
                  {generatingSampleData ? '생성 중...' : '샘플 데이터 생성'}
                </button>
                <button
                  onClick={handleActivateSampleService}
                  disabled={togglingService === 'sample'}
      ...
      <div className="space-y-4">
        {services.map((service) => {
          const isSampleService = service.service_type === 'sample';
          return (
            <div
              key={service.config_id}
              className={`p-6 border rounded-lg hover:shadow-md transition-shadow ${
                isSampleService ? 'bg-amber-50 border-amber-200' : ''
              }`}
            >
              {/* Service Header */}
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${isSampleService ? 'bg-amber-100' : 'bg-blue-100'}`}>
                    <Database className={`w-6 h-6 ${isSampleService ? 'text-amber-600' : 'text-blue-600'}`} />
                  </div>
                  <div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t">
              {isSampleService ? (
                <>
                  <button
                    onClick={handleGenerateSampleData}
                    disabled={generatingSampleData || !allServicesDisabled}
                    className="flex items-center gap-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:bg-gray-400 transition-colors"
                    title={allServicesDisabled ? '샘플 데이터 생성' : '모든 ERP 서비스가 비활성화된 경우에만 사용 가능'}
                  >
                    <Play className="w-4 h-4" />
                    {generatingSampleData ? '생성 중...' : '샘플 데이터 생성'}
                  </button>
                  {service.is_enabled ? (
                    ...
                    <button
                      onClick={() => handleActivateSampleService()}
                      disabled={togglingService === 'sample' || !allServicesDisabled}
                      className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-400 transition-colors"
                      title={allServicesDisabled ? '샘플 서비스 활성화' : '모든 ERP 서비스가 비활성화된 경우에만 사용 가능'}
                    >
                      <ToggleRight className="w-4 h-4" />
                      {togglingService === 'sample' ? '처리 중...' : '활성화'}
                    </button>
                  )}
                </>
              ) : service.is_enabled ? (
