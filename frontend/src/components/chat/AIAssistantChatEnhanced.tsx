...
const AIAssistantChatEnhanced: React.FC<AIAssistantChatEnhancedProps> = ({ onExecuteSQL, className = '' }) => {
  ...
  const [showAgentTrace, setShowAgentTrace] = useState<Set<string>>(new Set());
  ...
  // 추적 표시 토글
  const toggleAgentTrace = (messageId: string) => {
            ...
            {messages.map((message) => (
              ...
              >
                ...
                >
                    ...
                    <div className="px-4 py-2 border-b border-gray-200 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        {message.agentTrace && message.agentTrace.length > 0 && (
                          <button
                            onClick={() => toggleAgentTrace(message.id)}
                            className={`text-xs px-2 py-1 rounded transition-colors ${
                              showAgentTrace.has(message.id)
                  ...
                  {/* 에이전트 추적 뷰어 */}
                  {showAgentTrace.has(message.id) && message.agentTrace && (
                  ...
                  {/* 증거 뷰어 */}
                  {showEvidence.has(message.id) && message.evidence && (
                    <div className="border-t">
