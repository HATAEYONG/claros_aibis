...
class CostKPIViewSet(viewsets.ViewSet):
    """원가 KPI ViewSet"""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        from .kpi_engine import CostKPIEngine
        self.engine = CostKPIEngine()
...
def cost_breakdown_4m2e(request):
    """4M2E 코스 분석 데이터"""
    period_type = request.query_params.get('period_type', 'monthly')
    period_value = request.query_params.get('period_value', '2024-12')
...
@api_view(['GET'])
@permission_classes([AllowAny])
def cost_driver_analysis_api(request):
    """코스 드라이버 분석 데이터"""
    dimension = request.query_params.get('dimension', 'ALL')
    period_type = request.query_params.get('period_type', 'monthly')
    ...
    # 필터링
    if dimension != 'ALL':
        drivers = [d for d in drivers if d['dimension'] == dimension]
