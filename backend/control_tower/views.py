...
class KPIMonitorViewSet(viewsets.ViewSet):
    """KPI 모니터링 ViewSet - Phase 2"""
    ...
    def dashboard(self, request):
        """KPI 대시보드 조회"""
        domains = request.query_params.getlist('domains', ['financial', 'production', 'quality'])
        time_range = request.query_params.get('time_range', 'today')

        service = KPIMonitorService()
        data = service.get_kpi_dashboard(domains=domains, time_range=time_range)
    ...
    def kpis(self, request):
        """특정 도메인 KPI 조회"""
        domain = request.query_params.get('domain', 'financial')
        time_range = request.query_params.get('time_range', 'today')

        service = KPIMonitorService()
        kpis = service._get_domain_kpis(
            domain,
            *service._calculate_period(time_range)
        )
    ...
    def history(self, request):
        ...
        service = KPIMonitorService()
        history = service.get_kpi_history(domain, kpi_id, days)
    ...
    def alert(self, request):
        ...
        if not all([domain, kpi_id, current_value is not None, status]):
            return Response({
                'error': 'domain, kpi_id, current_value, and status are required',
            }, status=status.HTTP_400_BAD_REQUEST)

        service = KPIMonitorService()
        alert = service.create_kpi_alert(domain, kpi_id, current_value, status)
    ...
    def definitions(self, request):
        ...
        if domain:
            definitions = service.KPI_DEFINITIONS.get(domain, {})
        else:
            definitions = service.KPI_DEFINITIONS
...
class DrillDownViewSet(viewsets.ViewSet):
    """드릴다운 ViewSet - Phase 2"""
    ...
    def kpi(self, request):
        ...
        if not domain or not kpi_id:
            return Response({
                'error': 'domain and kpi_id are required',
            }, status=status.HTTP_400_BAD_REQUEST)

        service = DrillDownService()
        data = service.drill_down_kpi(domain, kpi_id, filters)
    ...
    def event(self, request):
        ...
        service = DrillDownService()
        data = service.drill_down_event(event_id, include_related)
    ...
    def available(self, request):
        ...
        if not domain:
            return Response({
                ...
                'available_domains': ['financial', 'production', 'quality'],
            }, status=status.HTTP_400_BAD_REQUEST)

        service = DrillDownService()
        drilldowns = service.get_available_drilldowns(domain)
