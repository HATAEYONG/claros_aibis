...
class ServiceMonitor:
    ...
    def get_service_info(self) -> Dict[str, Any]:
        ...
        return {
            'service': 'Claros MIS AI Dashboard',
            'version': '1.0.0',
...
def health_metrics_view(request):
    ...
    try:
        ...
        metrics_lines = []
        metrics_lines.append('# HELP claros_health_check_status Health check status (1=healthy, 0.5=warning, 0=critical)')
        metrics_lines.append('# TYPE claros_health_check_status gauge')

        for check_name, result in results.items():
            status_value = 1 if result.status == 'healthy' else (0.5 if result.status == 'warning' else 0)
            metrics_lines.append(f'claros_health_check_status{{check="{check_name}"}} {status_value}')
