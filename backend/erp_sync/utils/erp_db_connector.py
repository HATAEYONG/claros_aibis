...
from ai.sql_guard import validate_select_only
...
class PostgreSQLConnector(BaseDatabaseConnector):
    ...
    def connect(self):
        ...
        try:
            ...
            if not ERPConnectionConfig.can_attempt_connection(self.erp_source.source_code):
                logger.debug(f"[ERP] {self.erp_source.source_code} 연결 쿨다운 기간 중")
                raise Exception(f"Connection cooldown for {self.erp_source.source_code}")

            if self._connection is None or self._connection.closed:
                # 설정에서 타임아웃 값 가져오기
                config = ERPConnectionConfig.get_source_config(self.erp_source.source_code)
                ...
                # 연결 성공 기록
                ERPConnectionConfig.record_connection_success(self.erp_source.source_code)
            ...
            ERPConnectionConfig.record_connection_failure(self.erp_source.source_code, e)
            ERPConnectionConfig.log_connection_error(self.erp_source.source_code, e)
            raise
...
def execute_erp_query(erp_source, query: str, params: Optional[tuple] = None) -> List[Dict[str, Any]]:
    ...
    소스 타입(PostgreSQL/MSSQL/MySQL/Oracle/SQLite)에 관계없이 이 함수를 거치는
    모든 쿼리는 SQL Guard로 SELECT 전용 여부를 검증한다 — AI가 생성한 SQL이든
    기존 ERP 동기화 쿼리든 동일하게 적용된다.
    ...
    Raises:
        SQLGuardViolation: query가 단일 SELECT/WITH 문이 아닌 경우
    """
    safe_query = validate_select_only(query)
    connector = ERPDatabaseConnector.get_connection(erp_source)
    return connector.execute_query(safe_query, params)
