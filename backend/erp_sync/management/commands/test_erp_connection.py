...
class Command(BaseCommand):
    help = 'SAP ERP 데이터베이스 연결을 테스트합니다.'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE('SAP ERP 데이터베이스 연결 테스트'))
        self.stdout.write(self.style.NOTICE('=' * 60))
        ...
        if not all([server, database, user, password]):
            ...
            self.stdout.write('\n    예시:')
            self.stdout.write('    python manage.py test_erp_connection --server=192.168.1.100 --database=SAP --user=sa --password=xxx')
            return
