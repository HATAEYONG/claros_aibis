...
def register_domain_agents():
    ...
    try:
        ...
        # HR 도메인 (Phase 2)
        from ai.agents.domain.hr_intelligence_agent import HRIntelligenceAgent
        registry.register(HRIntelligenceAgent())
        logger.info("HRIntelligenceAgent registered - Phase 2")

    except Exception as e:
        logger.error(f"Failed to register domain agents: {str(e)}")


def register_orchestration_agents():
    """오케스트레이션 레이어 에이전트 등록 (L1) - Phase 2"""
    try:
        from ai.agents.orchestration.tool_router_agent import ToolRouterAgent

        registry.register(ToolRouterAgent())

        logger.info("Orchestration agents (L1) registered - Phase 2")
    except Exception as e:
        logger.error(f"Failed to register orchestration agents: {str(e)}")
...
def register_analysis_agents():
    ...
    try:
        ...
        # OptimizationAgent (Phase 2)
        from ai.agents.analysis.optimization_agent import OptimizationAgent
        registry.register(OptimizationAgent())
        logger.info("OptimizationAgent registered - Phase 2")
...
def register_all_agents():
    ...
    # 오케스트레이션 레이어 (L1) - Phase 2
    register_orchestration_agents()
