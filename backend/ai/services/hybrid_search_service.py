...
from ai.vector_store import get_vector_store, get_embedding_generator
...
class HybridSearchEngine:
    """
    하이브리드 검색 엔진 (Phase 1 업그레이드)
    벡터 검색(pgvector 지원)과 키워드 검색을 결합하여 최적의 결과 제공
    """

    def __init__(self):
        self.vector_weight = getattr(settings, 'RAG_CONFIG', {}).get('VECTOR_WEIGHT', 0.7)
        self.keyword_weight = getattr(settings, 'RAG_CONFIG', {}).get('KEYWORD_WEIGHT', 0.3)
        self.rerank_top_k = getattr(settings, 'RAG_CONFIG', {}).get('RERANK_TOP_K', 20)
        self.top_k = getattr(settings, 'RAG_CONFIG', {}).get('TOP_K_RESULTS', 5)

        # 벡터 스토어 및 임베딩 생성기 초기화
        embedding_dimension = 1536  # OpenAI text-embedding-3-large
        self.vector_store = get_vector_store(embedding_dimension=embedding_dimension)
        self.embedding_generator = get_embedding_generator(embedding_dimension)
        ...
        for result in vector_results:
            ...
            if filters:
                ...
                if 'content_type' in filters:
                    if chunk.document.content_type != filters['content_type']:
                        continue
                if 'source_type' in filters:
                    if chunk.document.source_type != filters['source_type']:
                        continue
                if 'document_id' in filters:
                    if str(chunk.document.doc_id) != str(filters['document_id']):
                        continue
    ...
    def _get_query_embedding(self, query: str) -> Optional[List[float]]:
        ...
        Returns:
            임베딩 벡터
        """
        # Phase 1: 내장 임베딩 생성기 사용
        # Phase 2: OpenAI Embedding API 연동 가능
        return self.embedding_generator.generate(query)
...
class RetrievalAugmentedGenerator:
    ...
    def __init__(self):
        ...
        self.max_context_length = getattr(settings, 'RAG_CONFIG', {}).get('MAX_CONTEXT_LENGTH', 4000)
        self.max_chunks = getattr(settings, 'RAG_CONFIG', {}).get('TOP_K_RESULTS', 5)

        # LLM 클라이언트 (선택적)
        try:
            from ai.llm import get_claude_client
    ...
    def _generate_answer(
        ...
        **kwargs
        ...
        # LLM이 활성화된 경우 Claude 사용
        if self.llm_enabled and self.llm_client:
            try:
                from ai.llm import Message, format_prompt
                from ai import get_prompt_template_registry

                # 프롬프트 템플릿 사용
                registry = get_prompt_template_registry()

                # RAG 전용 프롬프트 생성
                system_prompt = """당신은 제조업 경영정보시스템(MIS)의 AI 어시스턴트입니다.
제공된 문서를 바탕으로 정확하고 유용한 답변을 제공해주세요.

답변 작성 시:
1. 문서 내용을 기반으로 답변하세요
2. 확실하지 않은 정보는 "추정"임을 명시하세요
3. 가능한 구체적인 수치와 데이터를 인용하세요
4. 한국어로 답변하세요"""
                ...
                logger.error(f'Claude LLM 답변 생성 실패: {e}')
