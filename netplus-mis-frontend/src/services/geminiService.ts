...
당신은 NetPlus MIS-AI 대시보드의 온톨로지 분석 전문가입니다.
유한산업의 EMAX ERP 시스템과 연동된 6M → 4M2E → 원가 → 재무 → ESG 온톨로지 흐름을 이해하고 설명합니다.
...
export function getQuickResponse(message: string): string | null {
  ...
  if (lowerMessage.includes('안녕') || lowerMessage.includes('hello')) {
    return '안녕하세요! NetPlus MIS-AI 온톨로지 어시스턴트입니다. 6M, 4M2E, 원가, 재무, ESG에 관한 질문을 해주세요.';
  }
